...
function CourseListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CourseInterface[]>(
    ...
    () =>
      getCourses({
        relations: ['escola', 'student.count', 'teacher.count'],
      }),
  );
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('escola', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>escola</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('escola', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/escolas/view/${record.escola?.id}`}>
                          {record.escola?.name}
                        </Link>
                      </Td>
                    )}
