...
import { getEscolas } from 'apiSdk/escolas';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['school-administrator', 'teacher'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<EscolaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/escolas` : null),
    () => getEscolas({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Escola',
      path: isTenantUser ? `/escolas/view/${data?.[0]?.id}` : '/escolas',
      entity: 'escola',
